...
- **resources/**: MCP resources (best practices guide, TypeScript type definitions); embeds `*.md` directly
- **dist/**: Embedded TypeScript type definitions (`dist/embed.go`); generated by `make prepare`
...
- **Embedded assets**: Documentation sections (`dist/sections.json`), markdown docs (`dist/markdown`), TypeScript types (`dist/embed.go`), and prompt/resource markdown files embedded at build time using `go:embed`
- **k6 subcommand**: Build a custom k6 binary with `make prepare && xk6 build --with github.com/grafana/mcp-k6=.`; the extension registers as `k6 x mcp`
